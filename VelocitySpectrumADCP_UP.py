# -*- coding: utf-8 -*-
"""
Created on Tue Sep 15 15:59:46 2026

@author: WangX3
"""

# -*- coding: utf-8 -*-
"""ADCP velocity spectra and IG/SS RMS for height-selected cells.

Reads a rotated ADCP Parquet file, selects N cells approximately equally
spaced over a user-defined height range, computes burst-resolved velocity
spectra/statistics, and plots the results. No output files are written.
"""
...
# ------------------------- USER SETTINGS -------------------------
input_file = Path(
    r"C:\dev\Python\LongWaveAnalysis\Processed\DVN_F3_ADCP_UP_rotated.parquet"
)
case_id = "DVN_F3_ADCP_UP"
case_label = "DVN F3 ADCP UP"

output_folder = Path(r"C:\dev\Python\LongWaveAnalysis\Spectra")
output_folder.mkdir(parents=True, exist_ok=True)

# Select N cells approximately equally spaced over this height range.
cell_height_min_m = 3.0
cell_height_max_m = 10.0
N_cell = 4
...
ig_low_hz, ig_high_hz = 0.005, 0.04
ss_low_hz, ss_high_hz = 0.04, 1.0

# --------------------------- FUNCTIONS ---------------------------
def identify_velocity_columns(columns):
    """
    Discover paired rotated cross-shore/alongshore velocity columns.

    Supports the existing naming convention:
      cross_shore_cellXX_idYY_zpNNNmm_m_s
      alongshore_cellXX_idYY_zpNNNmm_m_s

    The same convention is produced by the ADCP/ADCP-HR rotation script.
    """
    pat = re.compile(
    ...
    cells = [
        ...
        if {"cross_shore", "alongshore"} <= d.keys()
    ]

    if not cells:
        raise KeyError(
            "No paired rotated cross-shore/alongshore velocity "
            "columns were found."
        )

    return sorted(cells, key=lambda d: d["z_bin_m"])
...
def select_cells_by_height(
    ...
    """
    Select approximately equally spaced measured cells by height.

    N target heights are generated with np.linspace(min, max, N).
    For each target, the nearest still-unused measured cell is selected.
    """
    ...
    eligible = [
        ...
        if (
            np.isfinite(c["z_bin_m"])
            and minimum_height_m <= c["z_bin_m"] <= maximum_height_m
        )
    ]

    if not eligible:
        available = np.array(
            [c["z_bin_m"] for c in all_cells],
            dtype=float,
        )
        raise ValueError(
            "No cells lie inside the requested height range "
            f"[{minimum_height_m}, {maximum_height_m}] m. "
            f"Available range is "
            f"[{np.nanmin(available):.3f}, "
            f"{np.nanmax(available):.3f}] m."
        )

    eligible = sorted(
        eligible,
        key=lambda c: c["z_bin_m"],
    )
    ...
    if number_to_select >= len(eligible):
        return eligible
...
print(
    f"Selected {len(cells)} of {len(all_cells)} available ADCP cells "
    f"between {cell_height_min_m:.2f} and "
    f"{cell_height_max_m:.2f} m."
)
print("Target heights (m):", np.round(target_heights, 3))

print("Cells used for spectral analysis:")
for c in cells:
    print(
        f"  Source cell {c['cell_number']}: "
        f"z={c['z_bin_m']:.3f} m"
    )
...
# ----------------------------- NO FILE OUTPUT -----------------------------
# Results remain available in memory as:
#   statistics : all accepted/rejected burst-cell statistics
#   accepted   : accepted burst-cell statistics
#   spectra    : burst-resolved Suu and Svv spectra
#   directional: detrended u/v time series for accepted bursts
#
# No CSV or Parquet files are written by this script.
...
if not spectra.empty:
    ...
    def plot_aggregate_velocity_spectra(statistic):
        ...
        fig.suptitle(
            f"{statistic.capitalize()} ADCP velocity autospectra "
            f"({case_label})"
        )
...
# ============================================================
# RMS TIME SERIES — ALL ACCEPTED BURSTS
# ============================================================
...
def plot_vertical_rms(column, title, ylabel):
    fig, axes = plt.subplots(
        len(cells),
        1,
        figsize=(12, 2.2 * len(cells)),
        sharex=True,
    )

    axes = np.atleast_1d(axes)

    # Common y-axis limit for vertical comparison
    ...
    for ax, c in zip(axes, cells):
        ...
        d = (
            accepted.loc[
                accepted["cell_number"] == c["cell_number"]
            ]
            .sort_values("mid_time")
            .copy()
        )

        # Plot all accepted bursts directly.
        # No artificial gap masking.
        ax.plot(
            d["mid_time"],
            d[column],
            linewidth=1.0,
        ...
        ax.set_ylim(
            0,
            1.05 * ymax if np.isfinite(ymax) else None,
        )
...
plot_vertical_rms(
    ...
    "Vertical consistency of IG velocity RMS",
...
plot_vertical_rms(
    "horizontal_ss_rms_m_s",
    "Vertical consistency of sea-swell velocity RMS",
    r"$|\mathbf{U}_{\mathrm{RMS,SS}}|$" "\n(m/s)",
)


# ============================================================
# IG / SS VELOCITY-VARIANCE RATIO
# Highest selected cell
# ============================================================
...
d["R_u"] = (
    d["horizontal_ig_rms_m_s"] ** 2
    /
    d["horizontal_ss_rms_m_s"] ** 2
)
...
ax.set_title(
    f"IG/SS velocity-variance ratio — "
    f"Cell {highest['cell_number']}, "
    f"z={highest['z_bin_m']:.3f} m "
    f"({case_label})"
)
...
#--------SAVE----------#
burst_file = output_folder / "all_cases_ADCP_UP_velocity.csv"  
save_cols = [
